...
- I wrote a [primitive scraper](https://github.com/clintonboys/gumtree_scraper){target="_blank"} for trading post website gumtree.com using the Python packages BeautifulSoup and requests, which persistently searches the site for a given query and sends an email using Mandrill when there are new results (for example when you're after concert tickets). 
